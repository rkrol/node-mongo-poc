...
function setDetailsLinkUrl(){
    $("#details_link").attr('href', encodeURIComponent('request_get_request.html'));
}

function setDetailsMenu(){
    var menu = $(".nav ul");
    $("<a>").attr({href: encodeURIComponent('request_get_request.html')}).text('GET request').wrap("<li>").parent().appendTo(menu);
}
